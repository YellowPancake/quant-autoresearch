...
测试期为2023年至2026年9月11日；展示结果均统一计入分红再投资，下一交易日收盘成交，
买卖各0.03%费用，按日历天数年化。这些是固定历史策略的统一口径复评。

### 宽基指数 · 最新实验

在24个指数及ETF代理构成的候选池中筛选标的、研究组合配置。
20小时内完成488次尝试，产生18个历史验证冠军节点。
下图比较事后测试收益最高的 #162、创业板买入持有与沪深300买入持有，净值均从1开始。

![宽基策略、创业板与沪深300买入持有的测试期净值对比](examples/studies/total_return/broad_market/figures/test_equity.png)

#162：测试累计收益 **+172.52%**，年化收益 **31.24%**，最大回撤 **30.05%**。
同期创业板ETF买入持有累计收益 **+47.32%**、最大回撤 **40.88%**；沪深300全收益买入持有累计收益 **+28.13%**、最大回撤 **22.41%**。
...
![沪深300策略与满仓持有的测试期净值对比](examples/studies/total_return/csi300/figures/test_equity.png)

策略 #84：累计收益 **+77.30%**，满仓持有 **+28.13%**；
最大回撤分别为 **10.96%**、**22.41%**。两组实验使用同一条沪深300全收益基准。
该曲线同样展示事后比较中测试收益最高的历史验证冠军节点。

### 最终冻结策略

下表展示验证集选出、在最终测试前冻结的策略，按统一口径复评，没有重新调参：

| 实验 | 尝试次数 | 冻结策略 | 测试年化收益 | 测试最大回撤 |
|---|---:|---|---:|---:|
| 宽基指数 | 488 | #478 | 23.33% | 31.00% |
| 沪深300 | 178 | #178 | 12.93% | 11.59% |
